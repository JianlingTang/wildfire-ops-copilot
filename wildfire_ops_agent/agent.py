...
action_agent = LlmAgent(
    ...
    instruction=(
        ...
        "- Write a custom_draft that follows the user's requested focus and wording, using current context when available. "
...
root_agent = LlmAgent(
    ...
    instruction=(
        ...
        "decide whether a workflow tool is required, and return a concise operator-facing answer. Do not invent tools or data.\n"
        "\n"
        "Decision policy:\n"
        "- Direct context-only answers are allowed only for factual lookups such as AOI center, radius, selected state, "
        "latest run id, risk score, report metadata, or existing Elastic evidence file names already present in context_json.\n"
        "- Operational judgment and evidence synthesis questions must use their workflow tool even when context_json contains useful facts. "
        ...
        "'which area should we inspect first?', 'what exposed assets are in the AOI?', recommendations, or next operational steps.\n"
        ...
        "7. monitor_task_tool: use when the user asks to create a monitor task, monitor a state/AOI at a requested cadence, "
        ...
        "- Context-only answers must not create records, approvals, reports, monitors, visualizations, or external actions.\n"
